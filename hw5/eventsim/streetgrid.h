...
private:
    ...
    //if rowDir = true, gives time delay going eastbound through row segment. if
    //rowDir = false, gives time delay going southbound through coloumn segment
    size_t timeDelay(size_t row, size_t col, bool rowDir);
    ...
    //model row segments
    size_t** rows;
    //model column segments
    size_t** columns;
    //INTERSECTION 2D array to see what last direction of ties went
    bool** ties;
    //checks blocks on segments
    bool** rowBlocks;
    bool** colBlocks;
    //street grid of vehicles
    std::map<std::string, bool> grid;
