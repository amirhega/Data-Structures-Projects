...
/**
 * Simple test case where the program can choose both items.  This just checks the number of items returned.
 */
TEST(SuperSimple, ChooseTwoItems)
{
	std::vector<GItem> items = { GItem(1, 5, 3), GItem(2, 2, 2) };
	std::vector<GItem> result = maxGroceryBox(items, 8);
...
TEST(CorrectSelection1, FindsBetter3Items1)
{
	std::vector<GItem> items = { GItem(1, 5, 3), GItem(2, 2, 2), GItem(3, 1, 1), GItem(4, 2, 2), GItem(5, 2, 2) };
	std::vector<GItem> result = maxGroceryBox(items, 6);

	EXPECT_TRUE(checkGroceryBox({ GItem(2, 2, 2), GItem(4, 2, 2), GItem(5, 2, 2), }, result));
}
